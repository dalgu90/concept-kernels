...
class Tokenizer(nn.Module):
    ...
    def forward(self, x_num: Tensor, x_cat: ty.Optional[Tensor]) -> Tensor:
        ...
        if x_cat is not None:
            if x_cat.shape[1] == len(self.category_offsets):
                # When ordinal indexes (token_idxs) are given directly
                token_idxs = x_cat
            else:
                # When one-hot encodings are given
                assert x_cat.shape[1] == self.category_embeddings.weight.shape[0]
                x_cat_row_sum = x_cat.sum(dim=-1)
                assert torch.all(x_cat_row_sum == x_cat_row_sum[0])
                _, cols = torch.nonzero(x_cat, as_tuple=True)
                token_idxs = cols.reshape(x_cat.shape[0], -1)
            x = torch.cat(
                [x, self.category_embeddings(token_idxs)],
                dim=1,
            )
            # x = torch.cat(
