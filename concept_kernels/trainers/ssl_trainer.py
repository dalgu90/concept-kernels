...
from concept_kernels.datasets.ssl_dataset import (
    SSLTransitionDataset,
    # SSLTransitionDataset2,
    # SSLTransitionDataset3,
    # SSLTransitionDataset4,
    # SSLTransitionDataset5,
    # SSLTransitionDataset6,
    # SSLTransitionDataset7,
)
...
class SSLTrainer:
    ...
    def train(self, model, train_dataset, val_dataset=None):
        ...
        if self.config.phase == 'finetune':
            ...
            logger.info("Setting model.phase to finetune")

        # Preprocessing
        if not train_dataset.is_preprocessed:
            model.data_preproc(train_dataset)
        if not val_dataset.is_preprocessed:
            model.data_preproc(val_dataset)

        # Augmentation
        if self.config.phase == 'pretrain':
            if self.config.data_augment.name == 'transition':
                SSLDatasetCLS = SSLTransitionDataset
            # elif self.config.data_augment.name == 'transition2':
                # SSLDatasetCLS = SSLTransitionDataset2
            # elif self.config.data_augment.name == 'transition3':
                # SSLDatasetCLS = SSLTransitionDataset3
            # elif self.config.data_augment.name == 'transition4':
                # SSLDatasetCLS = SSLTransitionDataset4
            # elif self.config.data_augment.name == 'transition5':
                # SSLDatasetCLS = SSLTransitionDataset5
            # elif self.config.data_augment.name == 'transition6':
                # SSLDatasetCLS = SSLTransitionDataset6
            # elif self.config.data_augment.name == 'transition7':
                # SSLDatasetCLS = SSLTransitionDataset7
            else:
                raise ValueError(f"Wrong augmentation name: {self.config.data_augment.name}")
            train_dataset = SSLDatasetCLS(train_dataset, **self.config.data_augment.params)
            val_dataset = SSLDatasetCLS(val_dataset, **self.config.data_augment.params)

        # Load pretrained encoder & freeze
        if self.config.phase == 'finetune':
            # Checkpoint saver
            pretrain_saver = load_checkpoint_saver(self.config.pretrain_saver)
            ckpt_epoch, ckpt_fname = pretrain_saver.get_latest_checkpoint()
            pretrain_saver.load_ckpt(
                model=model, optimizer=None, ckpt_fname=ckpt_fname,
                ignore_modules=['predictor']
            )
            logger.info(f"Pretrained encoder loaded from {self.config.pretrain_dir}/{ckpt_fname}")
        ...
        for epoch in range(init_epoch, max_epochs):
            ...
            for batch_train in train_loader:
                ...
                batch_loss, batch_outputs, _ = model_train(
            ...
            if val_dataset:
                val_outputs, val_labels, val_features, val_loss = self._forward_epoch(
    ...
    def evaluate(self, model, dataset=None, dataloader=None, return_output=False):
        ...
        epoch_outputs, epoch_labels, epoch_features = self._forward_epoch(
        ...
        if return_output:
            return metric_vals, (epoch_outputs, epoch_labels, epoch_features)
        else:
            return metric_vals
    ...
    def _forward_epoch(self, model, dataset=None, dataloader=None,
        ...
        model.eval()
        epoch_outputs, epoch_labels, epoch_losses, epoch_features = [], [], [], []
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                if compute_loss:
                    loss, batch_outputs, batch_features = model(batch,
                                                                compute_loss=True,
                                                                return_output=True)
                    epoch_losses.append({k: v.cpu() for k, v in loss.items()})
                else:
                    batch_outputs, batch_features = model(batch, compute_loss=False)
                epoch_labels.append(batch['label'].cpu())
                epoch_outputs.append(batch_outputs.cpu())
                if batch_features is not None:
                    epoch_features.append(batch_features.cpu())

        # Concat / mean
        epoch_labels = torch.cat(epoch_labels, 0)
        epoch_outputs = torch.cat(epoch_outputs, 0)
        if epoch_features:
            epoch_features = torch.cat(epoch_features, 0)
        else:
            epoch_features = None
        ret = (epoch_outputs, epoch_labels, epoch_features)
        if compute_loss:
